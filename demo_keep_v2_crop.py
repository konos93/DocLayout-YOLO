...
def main():
    ...
    for idx, res in enumerate(det_res):
        ...
        if boxes is None or len(boxes) == 0:
            ...
            # Still write a fallback crop so downstream code finds something
            ...
            if img is not None:
                ...
                fallback_path = os.path.join(args.res_path, fallback_name)
                cv2.imwrite(fallback_path, crop)
                # Also save a blank kept-only image with the small region visible
                mask = np.zeros(img.shape[:2], dtype=np.uint8)
                mask[0:h_crop, 0:w_crop] = 255
                kept_only = cv2.bitwise_and(img, img, mask=mask)
                cv2.imwrite(kept_only_path, kept_only)
                print(f"[{base}] No kept detections. Saved fallback 30x30 top-left crop: {fallback_path}")
            continue
        ...
        for i, (x1, y1, x2, y2) in enumerate(xyxy):
            if cls[i] in keep_ids:
                ...
                # NEW filename format: crop_abandon01___IMG_0447.JPG
        ...
        # Fallback if no crops were kept for this image
        if len(kept) == 0:
            h, w = img.shape[:2]
            w_crop = min(30, w)
            h_crop = min(30, h)
            ...
            kept.append({
                "cls_id": -1,
                "cls_name": "topleft_fallback",
                "conf": 1.0,
                "bbox_xyxy": [0, 0, w_crop, h_crop],
        ...
        # Keep the original summary line semantics
        kept_names_norm = [normalize(n) for n in keep_names]
        saved_crops_count = sum(1 for k in kept if normalize(k['cls_name']) in kept_names_norm)
        print(f"[{base}] Annotated: {annotated_path}")
        print(f"[{base}] Kept only page: {kept_only_path}")
        if args.save_json:
            print(f"[{base}] JSON: {json_path}")
        print(f"[{base}] Saved crops: {saved_crops_count}")
        total_kept += len(kept)
